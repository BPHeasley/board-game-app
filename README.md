...
```bash
board-game-app$ sam build --use-container
```
...
```bash
board-game-app$ sam local invoke HelloWorldFunction --event events/event.json
```

The SAM CLI can also emulate your application's API. Use the `sam local start-api` to run the API locally on port 3000.

```bash
board-game-app$ sam local start-api
board-game-app$ curl http://localhost:3000/
```
...
```bash
board-game-app$ sam logs -n HelloWorldFunction --stack-name board-game-app --tail
```
...
```bash
board-game-app$ pip install -r tests/requirements.txt --user
# unit test
board-game-app$ python -m pytest tests/unit -v
# integration test, requiring deploying the stack first.
# Create the env variable AWS_SAM_STACK_NAME with the name of the stack we are testing
board-game-app$ AWS_SAM_STACK_NAME="board-game-app" python -m pytest tests/integration -v
```
...
```bash
sam delete --stack-name "board-game-app"
```
